...
__webpack_require__(/*! ./pdfobject */ "./resources/js/pdfobject.js");
...
(function (root, factory) {
  ...
  var pdfobjectversion = "2.2.8";
  var nav = window.navigator;
  var ua = window.navigator.userAgent; //Time to jump through hoops -- browser vendors do not make it easy to detect PDF support.
  ...
  */
  ...
  var isModernBrowser = window.Promise !== undefined; //Older browsers still expose the mimeType

  var supportsPdfMimeType = nav.mimeTypes["application/pdf"] !== undefined; //Safari on iPadOS doesn't report as 'mobile' when requesting desktop site, yet still fails to embed PDFs

  var isSafariIOSDesktopMode = nav.platform !== undefined && nav.platform === "MacIntel" && nav.maxTouchPoints !== undefined && nav.maxTouchPoints > 1; //Quick test for mobile devices.

  var isMobileDevice = isSafariIOSDesktopMode || /Mobi|Tablet|Android|iPad|iPhone/.test(ua); //Safari desktop requires special handling 

  var isSafariDesktop = !isMobileDevice && nav.vendor !== undefined && /Apple/.test(nav.vendor) && /Safari/.test(ua); //Firefox started shipping PDF.js in Firefox 19. If this is Firefox 19 or greater, assume PDF.js is available
  ...
  var createAXO = function createAXO(type) {
    var ax;
  ...
  var supportsPdfActiveX = function supportsPdfActiveX() {
    return !!(createAXO("AcroPDF.PDF") || createAXO("PDF.PdfCtrl"));
  }; //Determines whether PDF support is available


  var supportsPDFs = //As of Sept 2020 no mobile browsers properly support PDF embeds
  ...
  var embedError = function embedError(msg, suppressConsole) {
    if (!suppressConsole) {
  ...
  var generatePDFObjectMarkup = function generatePDFObjectMarkup(embedType, targetNode, url, pdfOpenFragment, width, height, id, title, omitInlineStyles, PDFJS_URL) {
    ...
    var el_type = embedType === "pdfjs" || embedType === "iframe" ? "iframe" : "embed";
    ...
    if (!omitInlineStyles) {
      var style = el_type === "embed" ? "overflow: auto;" : "border: none;";
  ...
  var _embed = function embed(url, targetSelector, options) {
    ...
    var fallbackHTML = "";
    var pdfOpenFragment = "";
    var fallbackHTML_default = "<p>This browser does not support inline PDFs. Please download the PDF to view it: <a href='[url]'>Download PDF</a></p>"; //Ensure URL is available. If not, exit now.
